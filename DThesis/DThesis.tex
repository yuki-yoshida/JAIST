...
leaf cases reduce to true and then the proof goal is verified. The
proof development procedure provided by the framework prescribes what
kind of set of proof goals should be sufficient to verify invariant
and leads-to properties of specifications and guides how to split
cases to develop proof case trees. The procedure supports proof
developers to make the specific proof goals for their own
...
\label{def:sufcond}
  Let $TS=(\mathit{St},\mathit{Tr},In)$ be a state machine, $p$ and
  $q$ predicates of $\mathit{St}$, $inv$ an invariant of $TS$, and $m$
  a natural number function of $\mathit{St}$, then the following three
  conditions\footnote{However Futatsugi~\cite{Futatsugi15} proposes
    four conditions, we have discussed and concluded that they can be
    rearranged into three conditions.}  are sufficient for
  ($p~\mbstt{leads-to}~q$) to
  hold\footnote{Definition~\ref{def:leadsto} of
    ($p~\mbtt{leads-to}~q$) includes the case where there is an
    infinite transition sequence
    $\alpha_{\infty}\!=\!(\mathit{S_0},\mathit{S_1},\dots)$ such that
    predicate $q$ never holds in $\alpha_{\infty}$, $\forall
    \mathit{S_i}\!\in\!\alpha_{\infty}\!:\!\neg q(\mathit{S_i})$, but
    any prefix of $\alpha_{\infty}$ reaches a state where $q$ holds,
    $\forall \alpha_i\!=\!(\mathit{S_0},\dots,\mathit{S_i}),\exists
    T\!\in\!\mathit{St}, \exists\beta\!\in\!\Theta\!:\!(q(T) \land
    S\alpha_i\beta T\in \Theta))$.  However, {\tt leads-to} may be
    defined in a narrower sense where no such infinite transition
    sequence is allowed. Here, we call the former as {\it
      weak-leads-to} and the latter as {\it strong-leads-to}. The set
    of conditions proposed by Lemma~\ref{def:sufcond} is sufficient to
    both meanings of leads-to because the properly decreasing natural
    number function, $m$, ensures that transition sequences never
    become infinite while keeping $q$ not to hold. In the rest of this
    dissertation, we mean ($p~\mbtt{leads-to}~q$) as
    ($p~\mbtt{strong-leads-to}~q$).}.
...
\begin{lemma}[sufficient conditions for $init~\mbstt{leads-to}~final$]
  ...
  then the following five conditions are sufficient for
...
Condition~(1) means an initial state should be a continuing state,
i.e.\ it should start transitions. Condition~(2) means transitions
continue until $final(S')$ holds. Condition~(3) implies that $m(S)$
keeps to decrease properly while $final(S)$ does not hold. Since
$m(S)$ is a natural number, it should stop to decrease in finite steps
and the state machine should get to state $S'$ where $cont(S')$ does
not hold and $final(S')$ does hold, because condition~(2) ensures that
transitions would still continue if $cont(S')$ does hold. Here, $m$ is
called a {\it state measuring function}. When condition~(4) and~(5)
hold, each state predicate included in $inv$ is called an invariant.

The rest of this chapter explains the verification procedure for five
...
proving conditions (1) to (5) above and are introduced by the users of
...
conditions (2), (3), and (5) include $inv$ in their antecedent
...
As to sufficient conditions (4) and (5), $inv$ is also included in
...
\noindent{\bf Step 1-2:} Consider which rule can be applied to the global
...
\noindent{\bf Step 2-4:} Similarly as Step 1-2, consider which rule can
...
%% ===============================================================
\section{Procedure: Proof of Sufficient Condition~(4) \&~(5)}
\label{sec:invariant}
%% ===============================================================
...
Predicate {\tt initinv} for condition~(4) can be defined as follows:
%% =======================================================================
\small
\begin{verbatim}
  vars S : State
...
\noindent{\bf Step 4-1:} Instantiate proved lemmas for predefined
predicates. \\
...
\noindent{\bf Step 4-2:} Begin with the most general case. \\
...
\noindent{\bf Step 5-0:} Define a predicate to be proved. \\ Using the
double negation idiom, predicate {\tt invinv} for
condition~(5) can be defined as follows:
%% =======================================================================
\small
\begin{verbatim}
  vars S SS : State
...
\noindent{\bf Step 5-2:} Begin with the cases each of which matches to
LHS of each rule. \\ 
\noindent{\bf Step 5-3:} Split the current case for a rule into
cases where the condition of the rule does or does not hold. 
...
to Step 5-3. The first case should be split into more two cases
where the set of properties is or is not empty.
Then, the goals of all four cases can be proved and
thus condition~(5) is proved as follows:
%% ===============================================================
\small
\begin{verbatim}
...
condition~(5) for each of them. Most of them check some
...
\noindent{\bf Step 5-0:} Define a predicate to be proved. 
%% =======================================================================
\small
\begin{verbatim}
  var S : State
...
instantiated and used for proof of condition~(5) as
follows:
%% ===============================================================
\small
\begin{verbatim}
...
thus we only need to prove condition~(5) for $noCycle_C$.
...
\normalsize
%% =======================================================================
\noindent{\bf Step 1-2:} Consider which rule can be applied to the 
...
conditions (2) (3) and (5), the users of the framework should
...
there are five sufficient conditions for it.
...
natural number axioms to be used for proof of condition~(3).  Module {\tt
...
6 cases whereas (2) and (3) require 12 goals each of which
is for each of 12 transition rules. Condition (4) also requires 12
goals for each of 12 invariants and (5) requires 144 goals for
each combination of 12 invariants and 12 rules. We need 25
problem specific lemmas each of which requires one goal.
The verification consists of totally 206 goals which
are split into 918 cases.
...
\begin{table}[hbtp]
  ...
  \begin{tabular}{lrlr}
    ...
    Condition (4)  & 12 & (12 invariants) & 18  \\
    Condition (5)  & 144 & (12 rules $\times$ 12 invariants) & 443  \\
...
condition~(4) and (5) for invariants about
...
those of conditions (2)(3)(5) which should be proved for each of
twelve trans rules.
