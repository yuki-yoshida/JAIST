...
as {\it Puppet}~\cite{Puppet}, {\it Chef}~\cite{Chef}, and {\it
  Ansible}~\cite{Ansible}. Currently operation engineers have to learn
and use these several kinds of tools in actual situations, which
results in much elaboration to guarantee correctness of automated
operations. In an actual commercial experience of the author, more
than 50\% of troubles are caused by defects in those dependency
...
from lack of competent object oriented architects and Java programmer,
which is a major reason why some of them lost several million
...
used the SPIN model checker to verifiy two invariant properties of a
self-configuration procedure for autonomous networks of cellular base
stations.  Recently, Sala{\"u}n, G., et
al.~\cite{EtcheversCBP11,SalaunBCPEG13,SalaunEPBC13} designed a system
setup protocol in the cloud and demonstrated to verify a liveness
property of the protocol using their model checking method.
...
al.~\cite{KleinEHACDEEKNSTW09} also used the Isabelle/HOL theorem
...
proof steps. Since they focused functional correctness, they do not
...
scale of problem which is described as follows:
\vspace{-0.3cm}
\begin{quotation}
...
Silva, R., et al.~\cite{SilvaB09} proposed a generic instantiation
approach to reuse proofs for Event-B. Using the composing and renaming
plug-ins for the Rodin platform (a tool set for Event-B), they defined
a way of instantiating generic models and defined sufficient proof
obligations to ensure that the proofs in a generic development remain
valid in an instantiated development. Tikhonova, U., et
al.~\cite{TikhonovaMBAV13} applied this idea to verify LACE DSL
programs for controlling lithography machines. Although a part of
proofs for the general specification can be instantiated and reused,
the generated specifications are too large and the provers of Rodin
fail to discharge instantiated theorems. They say that they do not
...
understanding of proof strategies. Thus, they only checked structural
properties using an animation plug-in of Rodin.
...
the main concern of this paper.  One of the major contributions of
this paper is to introduce the idea of frameworks from software
development to proof development which has a unique set of features:
\vspace{-0.3cm}
\begin{itemize}
\setlength{\parskip}{0cm}
\setlength{\itemsep}{0cm}
\item our framework supports verification of leads-to property as well as invariant property,
\item uses interactive theorem proving to prove absence of counter examples,
\item provides abstract reusable entities and abstract proved lemmas for high productivity, and
\item provides a procedure to develop consistent proofs for high maintainability.
\end{itemize}
...
\noindent{\bf Step 1-2:} Think which rule is applied to the global
...
\normalsize
%% =======================================================================
\noindent{\bf Step 1-2:} Think which rule is applied to the 
...
The framework recommends to use \_ == \_ operator only in the
...
\item To keep the confluence property in the case splitting:\\ The
  ...
  and maintain. Thus, we decided to recommend to write proof codes
...
if the users of the framework go along with the following rule, then
...
\item A global state term included in the top goal of a proof should
  be a ground constructor term, e.g., \stt{< sRS,sPR >}.
\item When a current case is split and a constant in the current goal
  is defined as a more detailed term, it should be a ground
  constructor term, e.g., \stt{eq sRS = (res(trs',idRS,srs') sRS') .}
  Moreover, any pair of equations added for the case splitting do not
  have the same LHS.
...
carefully check the goal of proofs in Step 2-1, 2-2, 3-2, 3-3, 6-2,
and 6-3. A simple but effective way to check it is to remove the inner
``\stt{not}'' in the double negation idiom and see that the goal does
not reduce to true.
...
%% ===============================================================
There are very few existing researches on reuse of proofs. Silva, R.,
et al.~\cite{SilvaB09} proposed a generic instantiation approach to
reuse proofs for Event-B. Using the functionality to compose machines
(Shared Event Composition plug-in) and rename elements (Refactory
plug-in) of the Rodin platform, they defined a way of instantiating
generic models and defined sufficient proof obligations to ensure that
the proofs in a generic development remain valid in an instantiated
development. Tikhonova, U., et al.~\cite{TikhonovaMBAV13} applied this
idea to verify LACE DSL programs for controlling lithography
machines. They implemented transformation from LACE programs to
Event-B specifications which are composed using instantiated machines
from a general specification.  Although a part of proofs for the
general specification can be instantiated and reused, the generated
specifications are too large and the provers of Rodin fail to
discharge instantiated theorems. They say that they do not expect
their average user to prove these theorems using interactive provers,
as it requires knowledge of propositional calculus and understanding
of proof strategies. Thus, they only checked structural properties
using an animation plug-in, ProB, of Rodin.

However reusing abstract entities by renaming and composing is a
similar approach to our framework, Silva, R., et al. did not provide
any actually reusable entities whereas out framework provides general
templates, libraries, lemmas, and a procedure reusable in a specific
domain. Tikhonova, U., et al. applied the idea to the domain of the
lithography machine control, however, they did not reuse generally
proved properties to prove the instantiated specifications.
...
example in this paper, there are four main differences.
